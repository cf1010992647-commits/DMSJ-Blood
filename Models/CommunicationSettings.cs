...
namespace Blood_Alcohol.Models
{
    ...
    public class CommunicationSettings
    {
        ...
        /// PLC Modbus 从站站号
        /// </summary>
        public int PlcSlaveAddress { get; set; } = 1;
        ...
        public List<string> Validate()
        {
            ...
            if (PlcSlaveAddress <= 0 || PlcSlaveAddress > 247)
            {
                errors.Add("PLC 站号必须在 1-247 范围内");
            }

            if (TcpPort <= 0 || TcpPort > 65535)
            {
                errors.Add("TCP 监听端口必须在 1-65535 范围内");
            }

            if (string.IsNullOrWhiteSpace(TcpIP))
            {
                errors.Add("TCP 监听 IP 不能为空");
            }
        ...
        private void ValidateTcpDevices(List<string> errors)
        {
            ...
            for (int index = 0; index < TcpDevices.Count; index++)
            {
                ...
                if (device == null)
                {
                    errors.Add($"TCP 设备映射第 {index + 1} 行不能为空");
                    continue;
                }
                ...
                {
                    errors.Add($"TCP 设备映射 {rowName} 的 DeviceType 不能为空");
                }

                if (string.IsNullOrWhiteSpace(device.DeviceKey))
                {
                    errors.Add($"TCP 设备映射 {rowName} 的 DeviceKey 不能为空");
                }
                ...
                {
                    errors.Add($"TCP 设备映射 {rowName} 的 ClientIp 格式无效：{device.ClientIp}");
                }
            }
        }
    }
